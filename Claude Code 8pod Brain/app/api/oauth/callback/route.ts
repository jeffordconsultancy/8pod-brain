...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!state || state !== stored || !code || !provider || !workspaceId) {
      return NextResponse.redirect(new URL('/ctrl/connections?error=invalid', base), 302);
    }
    ...
    if (['gmail', 'google-drive', 'google-calendar'].includes(provider)) {
      ...
      return NextResponse.redirect(new URL('/ctrl/connections?error=unsupported', base), 302);
    }
    ...
    const res = NextResponse.redirect(new URL('/ctrl/connections?success=true', base), 302);
    ...
    return NextResponse.redirect(new URL('/ctrl/connections?error=failed', base), 302);
  }
}
