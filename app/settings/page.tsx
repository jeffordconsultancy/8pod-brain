  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-400">Manage your account settings and API keys</p>
      </div>

      {message && (
        <div className="bg-green-900/30 border border-green-700 text-green-200 px-4 py-3 rounded-lg">{message}</div>
      )}
      ...
      <div className="space-y-6">
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-white mt-1 font-mono text-xs">{(session as any)?.workspaceId || 'Not set'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
          ...
          {showNewKeyForm && (
            <form onSubmit={handleCreateKey} className="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-6">
          ...
          {apiKeys.length > 0 ? (
            <div className="space-y-3">
              {apiKeys.map((key) => (
                <div key={key.id} className="bg-gray-800 rounded-lg p-4 flex items-center justify-between">
                  <div className="flex-1">
                    <p className="font-medium text-white">{key.name}</p>
                    <p className="text-sm text-gray-400 mt-1 font-mono">{key.masked}</p>
                    {key.lastUsed && (
                      <p className="text-xs text-gray-500 mt-2">Last used: {new Date(key.lastUsed).toLocaleDateString()}</p>
                    )}
                  </div>
                  <button onClick={() => handleDeleteKey(key.id)}
                    className="px-3 py-1 text-sm font-medium text-red-400 hover:text-red-300">Delete</button>
                </div>
              ))}
            </div>
          ) : (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
          ...
          <div className="space-y-4">
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded bg-gray-800 border border-gray-700" defaultChecked />
