...
export async function POST(request: NextRequest) {
  try {
    ...
    // Simple relevance scoring: count query term matches in content
    ...
    const scored = allRecords.map(r => {
      ...
      // Boost recent records slightly
      const ageHours = (Date.now() - new Date(r.createdAt).getTime()) / (1000 * 60 * 60);
      score += Math.max(0, 1 - ageHours / (24 * 30)); // Small recency boost for last 30 days
      return { record: r, score };
    });

    // Sort by relevance score descending, take top 20
    scored.sort((a, b) => b.score - a.score);
    const relevantRecords = scored.slice(0, 20).map(s => s.record);

    // Build context from relevant records (max ~1000 chars each)
